...
export default function PhotoUpload({ propertyId, onPhotoUrl }) {
  ...
  if (preview) return (
    <div>
      <img src={preview} alt="Entry photo" style={{ borderRadius: 4, border: '1px solid var(--line)', maxHeight: 192, width: 'auto' }} />
      <button onClick={remove} style={{ marginTop: 8, fontSize: 11, fontFamily: 'monospace', color: 'var(--warn)', background: 'none', border: 'none', padding: 0, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Remove photo</button>
    </div>
  )
