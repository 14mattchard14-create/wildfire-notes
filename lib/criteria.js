...
export const GUIDED_SEGMENTS = [
  {
    ...
    wholeSidePhoto: true,
    // Shared with left/right/back — all four log into the same zone-level
    // plant list (0-5 FT. Noncombustible Zone), since a plant spotted from
    // any side is the same finding either way and the report merges by
    // zone, not by side. See property_plants (migration 019).
    plantsZone: '0-5 FT. Noncombustible Zone',
  ...
  },
  {
    ...
    wholeSidePhoto: true,
    plantsZone: '5-30 FT. Defensible Space - Vegetation',
...
// The distinct zones that get a Plants field (see `plantsZone` on the
// segments above and property_plants, migration 019) — vegetation is
// actually the finding in these three, unlike e.g. Roof or Vents. Multiple
// segments can share the same plantsZone (front/left/right/back all log
// into '0-5 FT. Noncombustible Zone'), so this list is deduplicated for
// callers that just need "which zones", like the report-draft prompt.
export const PLANT_ZONES = [...new Set(
  [...GUIDED_SEGMENTS, OVERALL_SITE_SEGMENT].map(s => s.plantsZone).filter(Boolean)
)]
