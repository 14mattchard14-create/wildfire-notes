...
// Shared shell for /users — same pattern as app/estimate/layout.js, but
// gated on isAdmin rather than just "any inspector account": this whole
// section (who can do what) is admin-only per PORTALS_AND_ROLES_PLAN.md.
export default function UsersLayout({ children }) {
  usePathname()
  const { user, loading, isAdmin, profileReady } = useAuth()
  ...
  if (!user || !isAdmin) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>Admin account required to view this page.</p>
    </div>
  )
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 20, background: 'var(--header-bg)', borderBottom: '1px solid var(--line)' }}>
        <div style={{ padding: '18px 20px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '0.02em', textTransform: 'uppercase', margin: 0, color: 'var(--header-text)' }}>Users &amp; Roles</h1>
          </div>
