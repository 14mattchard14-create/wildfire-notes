...
// Shared shell for /estimate — same pattern as app/insights/layout.js:
// centralizes the employee-only auth guard here instead of duplicating it
// in the page, uses the same AdminSidebar/header as every other admin
// section for a consistent shell.
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 20, background: 'var(--header-bg)', borderBottom: '1px solid var(--line)' }}>
        <div style={{ padding: '18px 20px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '0.02em', textTransform: 'uppercase', margin: 0, color: 'var(--header-text)' }}>Estimate</h1>
          </div>
